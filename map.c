...
)
{
   ...
   // Assume the seeds were not sorted.
   ...
   for (int mpos = end - LUTK ; mpos > leftmost->end+1 ; mpos--) {
      int c = ENCODE[(uint8_t) seq[mpos]];
      range.bot = get_rank(idx.occ, c, range.bot - 1);
      range.top = get_rank(idx.occ, c, range.top) - 1;
   }
   ...
   for (int mpos = leftmost->end+1 ; mpos+1 >= rightmost->beg ; mpos--) {
      for (int c = 0 ; c < 4 ; c++) {
...
//    leftmost MEM would be longer so as to abut the error (as in 2b).
//
//
//    |                        x=============  ...
//    |-----------------------------
...
//    Case 6: there is no error on the left of the MEM.
      ...
      int left_minscore, right_minscore;

      if (mem == leftmost) {
         // Cases 5 and 6.
         left_minscore = mem->beg == 0 ? 0 : 1;
      }
      else if (leftmost->beg == 0) {
         // Cases 1 and 2.
         left_minscore = leftmost->end >= mem->beg-2 ? 1 : 2;
      }
      else {
         // Cases 3 and 4.
         left_minscore = mem->beg >= GAMMA+1 ? 2 : 1;
      }

      if (mem == rightmost) {
         // Cases 5 and 6.
         right_minscore = mem->end == slen-1 ? 0 : 1;
      }
      else if (rightmost->end == slen-1) {
         // Cases 1 and 2.
         right_minscore = rightmost->beg <= mem->end+2 ? 1 : 2;
      }
      else {
         // Cases 3 and 4.
         right_minscore = mem->end <= slen-GAMMA-2 ? 2 : 1;
      }
      ...
      if (minscore > best_score || ((minscore == best_score) && (best->pos >= MAX_MINSCORE_REPEATS))) {
         continue;
      }
